...
export const convertToYup = (schema: Schema): Yup.ObjectSchema<Yup.AnyObject> => {
  const build = (key: string, config: Types, parent_path: string = ""): Yup.AnyObject => {
    ...
    if (config.type === "string") {
      ...
      if (config.object) {
        ...
        for (const [nested_key, nested_config] of Object.entries(config.object)) {
          nested_schema[nested_key] = build(nested_key, nested_config, field_path);
        }
