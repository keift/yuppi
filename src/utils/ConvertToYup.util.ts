...
export const convertToYup = (schema: Schema, options: YuppiOptions) => {
  const base = (schema: AnyObject, key: string, config: Type) => {
    ...
    if (!config.nullable && config.default !== null) schema = schema.nonNullable(({ path }: { path: string }) => (options.error_messages?.base?.nullable ?? '').replaceAll('{path}', path));

    if (config.required)
      schema = schema.test(
        'required',
        ({ path }: { path: string }) => (options.error_messages?.base?.required ?? '').replaceAll('{path}', path),
        (property: unknown) => {
    ...
    if (config.type === 'string') {
      ...
      if (config.enum)
        schema = schema.oneOf(
          ...
          ({ path }: { path: string }) => (options.error_messages?.string?.enum ?? '').replaceAll('{path}', path)
        );

      if (config.pattern !== undefined) schema = schema.matches(new RegExp(config.pattern), ({ path }: { path: string }) => (options.error_messages?.string?.pattern ?? '').replaceAll('{path}', path).replaceAll('{pattern}', config.pattern !== undefined ? new RegExp(config.pattern).source : ''));

      if (config.min !== undefined)
        schema = schema.min(config.min, ({ path, min }: { path: string; min: number }) =>
          (options.error_messages?.string?.min ?? '')
      ...
      if (config.max !== undefined)
        schema = schema.max(config.max, ({ path, max }: { path: string; max: number }) =>
          (options.error_messages?.string?.max ?? '')
            .replaceAll('{path}', path)
      ...
      if (config.min !== undefined) schema = schema.min(config.min, ({ path, min }: { path: string; min: number }) => (options.error_messages?.number?.min ?? '').replaceAll('{path}', path).replaceAll('{min}', String(min)));

      if (config.max !== undefined) schema = schema.max(config.max, ({ path, max }: { path: string; max: number }) => (options.error_messages?.number?.max ?? '').replaceAll('{path}', path).replaceAll('{max}', String(max)));

      if (config.integer === true) schema = schema.integer(({ path }: { path: string }) => (options.error_messages?.number?.integer ?? '').replaceAll('{path}', path));

      if (config.positive === true) schema = schema.positive(({ path }: { path: string }) => (options.error_messages?.number?.positive ?? '').replaceAll('{path}', path));

      if (config.negative === true) schema = schema.negative(({ path }: { path: string }) => (options.error_messages?.number?.negative ?? '').replaceAll('{path}', path));
      ...
      schema = Yup.date().typeError(({ path }: { path: string }) => (options.error_messages?.date?.type ?? '').replaceAll('{path}', path));

      if (config.min !== undefined) schema = schema.min(config.min, ({ path, min }: { path: string; min: string }) => (options.error_messages?.date?.min ?? '').replaceAll('{path}', path).replaceAll('{min}', new Date(min).toISOString()));

      if (config.max !== undefined) schema = schema.max(config.max, ({ path, max }: { path: string; max: string }) => (options.error_messages?.date?.max ?? '').replaceAll('{path}', path).replaceAll('{max}', new Date(max).toISOString()));
      ...
      if (config.min !== undefined)
        schema = schema.min(config.min, ({ path, min }: { path: string; min: number }) =>
          (options.error_messages?.array?.min ?? '')
            .replaceAll('{path}', path)
            .replaceAll('{min}', String(min))
            .replaceAll('{plural_suffix}', min > 1 ? 's' : '')
        );
      ...
      if (config.max !== undefined)
        schema = schema.max(config.max, ({ path, max }: { path: string; max: number }) =>
          (options.error_messages?.array?.max ?? '')
            .replaceAll('{path}', path)
            .replaceAll('{max}', String(max))
            .replaceAll('{plural_suffix}', max > 1 ? 's' : '')
        );
