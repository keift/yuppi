...
import { ValidationError, type Issue } from '../types/validation-error';
...
const validateTypeUnion = (types: TypeUnion, data: unknown, path: (string | number)[], options: YuppiOptions) => {
...
const validateType = (type: Type, data: unknown, path: (string | number)[], options: YuppiOptions) => {
  if (Array.isArray(type)) {
    return validateTypeUnion(type, data, path, options);
  } else return validateTypeSingle(type, data, path, options);
};

const validateSchemaSingle = (schema: SchemaSingle, data: unknown, path: (string | number)[], options: YuppiOptions) => {
...
const validateSchemaUnion = (schemas: SchemaUnion, data: unknown, path: (string | number)[], options: YuppiOptions) => {
