...
export const convertToJSONSchema = (schema: Schema): TObject => {
  const build = (key: string, config: Types): TSchema => {
    if (config.type === "string") {
      let schema: TString | TUnion<[TString, TNull]> = Type.String({ minLength: config.min, maxLength: config.max, pattern: (config.pattern ?? /[\s\S]*/).source, default: config.default });
      ...
      let schema: TString | TUnion<[TString, TNull]> = Type.String({ format: "date-time", minimum: config.min, maximum: config.max, pattern: (config.pattern ?? /[\s\S]*/).source, default: config.default });
